...
interet <- function(tabnum, tabbool) {
  matrice_interet <- matrix(NA, nrow = 2, ncol = 1)
  for (j in 1:1) {
...
condition <- function(tabnum, tabbool, carte, seuil, interet) {
  int <- interet[1,]
  if (carte == min(as.numeric(names(table(tabnum[which(tabbool[,1]==1),1])))) && interet[2,1]!=3) {
    #print("oui")
    pos<-0
    non_carte<-0
    pos <- which(tabnum[,1] == carte & tabbool[,1]==1, arr.ind = TRUE)# pos qui sont retournées dans la colonne j
    non_carte <- suppressWarnings(which(tabnum[,1] != tabnum[pos, 1] | tabbool[,1]==0, arr.ind = TRUE)) # pos qui ne sont pas la carte mais qui sont retournées (sinon c inutile)
    if (length(pos) == 2 && carte>0) {
      return(c(0, non_carte, 1))
    }
    else if (length(pos) == 1 && carte>0) {
      # on prend le premier des deux
      return(c(1, non_carte[1], 1))
    }
    else if (carte <= seuil && carte>0){
      place <- place(tabbool[,c(2:4)])
      return(c(2, place[1], place[2]+1))
    }
    else if (carte<=0 && length(pos)<2) {
      place <- maxpos(tabnum[,c(2:4)], tabbool[,c(2:4)])
      return(c(4, place[2], place[3]+1))
    }
    else{
      return(c(-1))
    }
    ...
    #print("non")
    if ((maxpos(tabnum[,c(2:4)], tabbool[,c(2:4)])[1] <= seuil) && sum(tabbool[,c(2:4)])<9) {
      place <- place(tabbool[,c(2:4)])
      return(c(3, place[1], place[2]+1))
    }
    else {
      place <- maxpos(tabnum[,c(2:4)], tabbool[,c(2:4)])
      return(c(4, place[2], place[3]+1))
  ...
  else if (carte > seuil && sum(tabbool[,1])<3) {
    place <- maxpos(tabnum[,c(2:4)], tabbool[,c(2:4)])
    return(c(5, place[2], place[3]+1))
  }
  else{
    return(c(-1))
  }
...
matrice <- matrix(c(2, 4, 4, 
                    2, 4, 4,
                    4, 0, 2, 
                    2, -2, 4), nrow = 3)

as.numeric(names(table(matrice[which(matbool[,1]==1),1])))
...
condition(matrice,matbool,4, 4,interet(matrice,matbool))
...
score_alpha_solodoub <- function(a=3,b=4, seuil){
  ...
  while(cpteur<stop && sum(tabbool)<12){
    ...
    if((defausse == min(as.numeric(names(table(tabnum[which(tabbool[,1]==1),1])))) && int[2,1]!=3) || defausse <= seuil){
      u <- defausse
    }
    else{
...
countcolseuil<-c()
for(i in -2:12){
  resultats<- replicate(100, score_alpha_solodoub_count(3,4, i))

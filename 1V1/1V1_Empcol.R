...
interet <- function(tabnum, tabbool) {
  ...
  for (j in 1:4) {
    ...
    if (length(valeurs_assoc)!=0){
      ...
      valeur_plusprez <- names(comptage)[which(comptage == 2)]
      if(length(valeur_plusprez)==1){
        matrice_interet[1, j] <- as.numeric(valeur_plusprez)
...
maxposEmpcol <- function(tabnum, tabbool,interet1) {
  # Trouver les indices où tabbool est égal à 1
  indices_avec_1 <- which(tabbool == 1, arr.ind = TRUE)
  valeurs_associees <- tabnum[indices_avec_1]
  #print(valeurs_associees)
  valeurs_associees_sort <- sort(valeurs_associees, decreasing = TRUE)
  #print(valeurs_associees_sort)
  for(i in 1:(length(valeurs_associees_sort)-1)){
    if(!(valeurs_associees_sort[i]%in%interet1[1,])){
      #print(valeurs_associees_sort[i])
      #print(indices_avec_1)
      indice_max<-which(valeurs_associees==valeurs_associees_sort[i])[1]
      ligne<-indices_avec_1[indice_max,1]
      colonne<-indices_avec_1[indice_max,2]
      valeur_max<-tabnum[ligne,colonne]
      return(c(valeur_max,ligne,colonne))
  ...
  indice_max<-which(valeurs_associees==valeurs_associees_sort[length(valeurs_associees_sort)])[1]
  ligne<-indices_avec_1[indice_max,1]
  colonne<-indices_avec_1[indice_max,2]
  valeur_max<-tabnum[ligne,colonne]
  return(c(valeur_max,ligne,colonne))
...
maxposEmpcol(matrice, matbool,interet(matrice, matbool))

conditionhybrideEmpcol <- function(tabnum, tabbool, carte, seuil,som1,cas1,tabnum1,tabbool1) {
  interet <- interet(tabnum, tabbool)
  interet1 <- interet(tabnum1, tabbool1)
  int <- interet[1,]
  j<-which(int==carte & interet[2,]==2)[1]
  new_matrix <- tabnum
  ...
  posmax <- maxposEmpcol(tabnum, tabbool,interet1)
  if (!(is.na(j))&&carte>0) {
    pos<-0
    non_carte<-0
    pos <- which(tabnum[,j] == carte & tabbool[,j]==1, arr.ind = TRUE)# pos qui sont retournées dans la colonne j
  ...
  while(bool1 && bool2){
    ...
    if(sum(tabbool2)!=12){
      ...
      if (fun2(tabnum2,tabbool2,defausse, seuil2)){
        a<-conditionj2(tabnum2,tabbool2,defausse,seuil2,som1,cas1,tabnum1,tabbool1)
        if(a[1]==-1){
          posi<-place(tabbool2)
        ...
        #print(v)
        a<-conditionj2(tabnum2,tabbool2,v,seuil2,som1,cas1,tabnum1,tabbool1)
        #print(4)
        if(a[1]==-1){
          posi<-place(tabbool2)
          tmp <- tabnum2[posi[1],posi[2]]
...
score_doubleRegscore(3,4,3,3,condhybride,condhybride,conditionhybride,conditionhybrideEmpcol)
...
hyb3_hyb3_Empcol<-sc_moy_totRegscore(10000, 3,3,condhybride,condhybride,conditionhybride,conditionhybrideEmpcol)
close(pb)
hyb3_hyb3_Empcol
